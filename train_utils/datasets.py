...
class BurgersLoader(object):
    def __init__(self, datapath, nx=2 ** 10, nt=100, sub=8, sub_t=1, pde_sub=8, pde_sub_t=1, new=False):
        dataloader = MatReader(datapath)
        self.sub = sub
        self.sub_t = sub_t
        self.s = nx // sub
        self.T = nt // sub_t
        self.pde_sub = pde_sub
        self.pde_sub_t = pde_sub_t
        self.pde_s = nx // pde_sub
        self.pde_T = nt // pde_sub_t
        self.new = new 
        if new:
            self.T += 1
            self.pde_T += 1
        ...
        self.pde_x_data = dataloader.read_field('input')[:, ::pde_sub]
        self.pde_y_data = dataloader.read_field('output')[:, ::pde_sub_t, ::pde_sub]

    def make_loader(self, n_sample, batch_size, start=0, train=True):
        Xs = self.x_data[start:start + n_sample]
        ys = self.y_data[start:start + n_sample]
        pde_Xs = self.pde_x_data[start:start + n_sample]
        pde_ys = self.pde_y_data[start:start + n_sample]
        ...
        if self.new:
            ...
            pde_gridx = torch.tensor(np.linspace(0, 1, self.pde_s + 1)[:-1], dtype=torch.float)
            pde_gridt = torch.tensor(np.linspace(0, 1, self.pde_T), dtype=torch.float)
        else:
            ...
            pde_gridx = torch.tensor(np.linspace(0, 1, self.pde_s), dtype=torch.float)
            pde_gridt = torch.tensor(np.linspace(0, 1, self.pde_T + 1)[1:], dtype=torch.float)
        gridx = gridx.reshape(1, 1, self.s)
        gridt = gridt.reshape(1, self.T, 1)
        pde_gridx = pde_gridx.reshape(1, 1, self.pde_s)
        pde_gridt = pde_gridt.reshape(1, self.pde_T, 1)
        ...
        pde_Xs = pde_Xs.reshape(n_sample, 1, self.pde_s).repeat([1, self.pde_T, 1])
        pde_Xs = torch.stack([pde_Xs, pde_gridx.repeat([n_sample, self.pde_T, 1]), pde_gridt.repeat([n_sample, 1, self.pde_s])], dim=3)
        dataset = torch.utils.data.TensorDataset(Xs, ys, pde_Xs, pde_ys)
        if train:
...
class DarcyFlow(Dataset):
    def __init__(self,
        ...
        if pde_sub == 1:
            self.pde_S = int(nx // pde_sub) 
        else:
            self.pde_S = int(nx // pde_sub) + 1
        ...
        self.pde_u = torch.tensor(u[offset: offset + num, ::pde_sub, ::pde_sub], dtype=torch.float)
    ...
    def __getitem__(self, item):
        fa = self.a[item]
        pde_fa = self.pde_a[item]
        return torch.cat([fa.unsqueeze(2), self.mesh], dim=2), self.u[item], torch.cat([pde_fa.unsqueeze(2), self.pde_mesh], dim=2), self.pde_u[item]
