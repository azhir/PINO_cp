...
def run(args, config):
    ...
    dataset = BurgersLoader(data_config['datapath'],
                            ...
                            pde_sub=data_config['pde_sub'], pde_sub_t=data_config['pde_sub_t'], new=True)
    ...
    train_2d_burger(model,
                    ...
                    padding=data_config['padding'], pde_padding=data_config['pde_padding'])
...
def test(config):
    ...
    if 'ckpt' in config['test']:
        ...
        model.load_state_dict(ckpt['model'])
        print('Weights loaded from %s' % ckpt_path)
    eval_burgers(model, dataloader, config, device, padding=data_config['padding'], pde_padding=data_config['pde_padding'])
